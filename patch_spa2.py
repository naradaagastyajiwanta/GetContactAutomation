...
from orchestrator.crm.tools import extract_text_from_html, gpt_extract_structured, ddg_search
...
async def social_post_analyzer_agent(state: CrmState) -> dict:
    ...
    if social.facebook_url:
        log.info("[SocialPostAnalyzer] Scraping Facebook timeline: %s via PinchTab", social.facebook_url)
        fb_html = await pt_fetch_html(social.facebook_url, timeout=30.0)
        if fb_html:
    ...
    if social.instagram_handle and social.instagram_handle != "None" and social.instagram_handle.lower() != "tidak ditemukan":
        ...
        log.info("[SocialPostAnalyzer] Scraping Instagram timeline: %s via PinchTab", ig_url)
        ig_html = await pt_fetch_html(ig_url, timeout=30.0)
        if ig_html:
    ...
    if social.twitter_handle and social.twitter_handle != "None" and social.twitter_handle.lower() != "tidak ditemukan":
        ...
        log.info("[SocialPostAnalyzer] Scraping Twitter timeline: %s via PinchTab", x_url)
        x_html = await pt_fetch_html(x_url, timeout=30.0)
        if x_html:
            collected_content["Twitter"] = extract_text_from_html(x_html, max_chars=6000)

    # Optional Fallback to DDG Search if standard scraping returns absolutely nothing
    if not collected_content:
        log.info("[SocialPostAnalyzer] Direct pinchtab scrape yielded no result, attempting post dorking...")
