...
import {
  ...
  triggerFindHandles,
  triggerScrapePosts,
  triggerExtractPhones,
  triggerDiscoverBem,
  triggerFindRectors,
  getProvinces,
  triggerCollectUniversities,
  getPipelineLogs,
  triggerAgentTargeted,
  pauseBot,
  resumeBot,
  type PipelineLogsParams,
  type TargetedAgentType,
} from "../api/pipeline";
...
export function useTriggerCollectUniversities() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (params?: { province?: string; limit?: number }) =>
      triggerCollectUniversities(params),
    onSuccess: (data) => {
...
export function useRunAgentTargeted() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      toast.success(
        data.message || `${AGENT_LABELS[variables.agentType]} started`,
      );
      queryClient.invalidateQueries({ queryKey: queryKeys.pipeline.status });
      queryClient.invalidateQueries({ queryKey: queryKeys.universities.all });
    },
