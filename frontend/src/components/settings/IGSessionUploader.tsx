    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex items-start gap-2.5 p-3 bg-blue-50 border border-blue-200 rounded-xl text-xs text-blue-800">
          ...
          <p>
            Paste value dari cookie <strong>sessionid</strong> yang sudah
            di-export dari browser.
          </p>
        </div>
...
export function IGSessionUploader() {
  ...
  const handleAddSession = async (sessionId: string) => {
    const newSessions = [
      ...sessions.map((s) => ({ ...s, visible: false })),
      {
  ...
  const saveSessions = async (newSessions: SessionItem[]) => {
    ...
    try {
      ...
      await apiClient.patch("/config/instagram", {
      ...
      <Card padding={false}>
        ...
        <div className="px-5 pb-5 space-y-3">
          ...
          <div className="flex items-start gap-2.5 p-3 bg-blue-50 border border-blue-200 rounded-xl text-xs text-blue-800">
            <Info className="w-4 h-4 mt-0.5 flex-shrink-0 text-blue-500" />
            <p>
              Tambahkan session cookie dari browser. Tidak perlu
              username/password. Multiple sessions membantu menghindari rate
              limit — sistem auto-rotates.
            </p>
          </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <code className="text-xs font-mono text-gray-700 dark:text-gray-300 truncate block">
                        {session.visible
                          ? session.value
                          : session.value.slice(0, 20) +
                            "..." +
                            session.value.slice(-10)}
                      </code>
          ...
          {sessions.length > 0 && (
            <p className="text-[10px] text-gray-400 text-center">
              Sessions auto-rotate when one gets rate-limited or suspended. Add
              2-3 sessions for best results.
            </p>
          )}
        </div>
      </Card>
