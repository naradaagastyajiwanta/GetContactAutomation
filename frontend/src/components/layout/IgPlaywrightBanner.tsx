...
export function IgPlaywrightBanner() {
  ...
  const disconnected = health.accounts.filter(
    (a) => a.status === "disconnected",
  );
  const authLimited = health.accounts.filter(
    (a) => a.status === "auth_limited",
  );
    ...
    try {
      ...
      if (fresh.all_ok) {
        ...
        showIGPopup({
          id: "ig-check-ok",
          type: "success",
          title: "Semua akun Instagram aktif",
          message:
            "Semua akun sudah terhubung dan siap digunakan untuk scraping.",
          duration: 6_000,
        });
      } else {
        const problemAccounts = fresh.accounts
        ...
        showIGPopup({
          id: "ig-check-bad",
          type: "error",
          title: `${fresh.total - fresh.connected} akun bermasalah`,
          message:
            "Beberapa akun Instagram tidak bisa digunakan untuk scraping. Lihat status masing-masing akun di bawah dan ambil tindakan yang diperlukan.",
      ...
      showIGPopup({
        ...
        message:
          "Tidak bisa terhubung ke server untuk mengecek status akun Instagram. Coba lagi beberapa saat.",
        duration: 8_000,
      });
    } finally {
      setChecking(false);
  ...
  return (
    ...
    >
      ...
      <div className="px-4 pb-3">
        ...
        {/* Actions */}
        <div
          className={`mt-2 flex items-center justify-between text-xs ${textColor}`}
        >
          <div className="flex items-center gap-2">
            {(disconnected.length > 0 ||
              authLimited.length > 0 ||
              hasBanned) && (
              <a
