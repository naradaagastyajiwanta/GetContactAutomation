...
function EmailLeftRail({
  ...
  campaignCounts: { draft: number; running: number; paused: number; completed: number; cancelled: number }
  inboxCount: number
  sentCount: number
  ...
  const campaignItems: NavItem[] = [
    ...
    { id: 'campaigns', label: 'Draft', icon: FileText, badge: campaignCounts.draft, section: 'campaigns', status: 'draft' },
    { id: 'campaigns', label: 'Active', icon: Rocket, badge: campaignCounts.running, section: 'campaigns', status: 'running' },
    { id: 'campaigns', label: 'Paused', icon: Pause, badge: campaignCounts.paused, section: 'campaigns', status: 'paused' },
    { id: 'campaigns', label: 'Completed', icon: CheckCircle2, badge: campaignCounts.completed, section: 'campaigns', status: 'completed' },
    { id: 'campaigns', label: 'Cancelled', icon: XCircle, badge: campaignCounts.cancelled, section: 'campaigns', status: 'cancelled' },
  ]
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto px-3 pb-3">
        ...
        <div>
          ...
          {campaignItems.slice(1).map((item) => {
            const Icon = item.icon
            const isActive = item.status ? activeStatusFilter === item.status : !activeStatusFilter
            return (
              <button
                key={`${item.id}-${item.label}`}
                onClick={() => onViewChange('campaigns', undefined, item.status)}
                className={cn(
                  'mb-0.5 flex w-full items-center gap-2.5 rounded-lg px-2 py-2 text-[13px] font-medium transition-all duration-150',
                  isActive
                    ? 'bg-indigo-600 text-white'
