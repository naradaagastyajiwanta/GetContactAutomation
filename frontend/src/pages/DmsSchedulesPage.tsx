...
import { DMS_SCHEDULES_TOUR_STEPS } from "../tours/dms-schedules.tour";
import {
  useDmsStats,
  useDmsSchedules,
  useDmsFollowups,
  useTriggerScheduleSync,
  useDmsResearchResults,
} from "../hooks/useDms";
...
import type {
  ScheduleSortKey,
  SortDir,
} from "../components/dms/DmsScheduleTable";
...
export default function DmsSchedulesPage() {
  ...
  const [sortKey, setSortKey] = useState<ScheduleSortKey | null>(
    "jadwal_audiensi",
  );
  ...
  const { data: schedulesData, isLoading: schedulesLoading } = useDmsSchedules(
    365,
    365,
  );
  ...
  usePageTour("dms-schedules", DMS_SCHEDULES_TOUR_STEPS);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div
          data-tour="dms-actions"
          className="flex flex-wrap items-center gap-2"
        >
          <button
            onClick={handleBulkResearch}
            disabled={
              researchProgress?.running || unresearchedUpcoming.length === 0
            }
          ...
          >
            <GraduationCap
              className={cn(
                "h-4 w-4",
                researchProgress?.running && "animate-pulse",
              )}
            />
            {researchProgress
