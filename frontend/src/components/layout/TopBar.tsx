...
import { saveLastPathBeforeLogout } from "../../lib/returnPath";
...
export function TopBar({ onMenuClick }: TopBarProps) {
  ...
  const logoutMutation = useMutation({
    mutationFn: logout,
    onMutate: () => {
      // Remember the current URL so that if the user logs back in on
      // the same tab, LoginPage can bounce them to where they were
      // (Q1b). sessionStorage-scoped — cleared automatically when the
      // tab closes, so "fresh browser launch" still starts at `/`.
      saveLastPathBeforeLogout();
    },
    onSuccess: async () => {
