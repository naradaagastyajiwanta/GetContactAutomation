...
type WSEvent =
  ...
  | {
      type: "email_recipient_updated";
      campaign_id: number;
      recipient: EmailBlastRecipient & { letter_number?: string | null };
    }
  | {
      type: "email_outbox_logged";
      email: SentEmail & { campaign_id?: number | null };
    }
  | {
      type: "email_inbox_received";
      email: InboundEmail;
      campaign_ids: number[];
    }
  | {
  ...
  | {
      type: "marketing_search_completed";
      group_id: number;
      total: number;
      found: number;
      not_found: number;
...
function upsertById<T extends { id: EmailCacheRowId | number }>(
  items: T[],
  item: T,
...
function updateEmailCampaignListCaches(
  ...
  const queries = qc
    .getQueryCache()
    .findAll({ queryKey: ["email-blast-campaigns"] });
  ...
  for (const query of queries) {
    ...
    qc.setQueryData(
      ...
      (
        ...
        const existing = old.campaigns.find(
        ...
        if (existing) {
          campaigns = campaigns
            .map((entry) =>
              entry.id === campaign.id ? { ...entry, ...campaign } : entry,
            )
...
function updateEmailRecipientCaches(
  ...
  const queries = qc
    .getQueryCache()
    .findAll({ queryKey: ["email-blast-recipients", campaignId] });
  ...
  for (const query of queries) {
    ...
    qc.setQueryData(
      ...
      (
        old:
          | { success: boolean; recipients: EmailBlastRecipient[] }
          | undefined,
      ) => {
        if (!old) {
          return old;
        ...
        const matchesFilter =
          !statusFilter || statusFilter === recipient.status;
...
function updateCampaignSentCaches(
  ...
  const queries = qc
    .getQueryCache()
    .findAll({ queryKey: ["email-blast-sent-emails", campaignId] });
  ...
  for (const query of queries) {
    ...
    qc.setQueryData(
      ...
      (
        old:
          | { success: boolean; emails: SentEmail[]; total: number }
          | undefined,
      ) => {
        if (!old) {
          return old;
  ...
  qc.setQueryData(
    ...
    (
      ...
      const exists = old.pages.some((page) =>
        page.emails.some((entry) => entry.id === email.id),
      );
      ...
      return {
        ...
        pages: [
          {
            ...firstPage,
            emails: upsertById(firstPage.emails, email, true).slice(
              0,
              firstPage.limit,
            ),
...
function updateCampaignInboxCaches(
  ...
  for (const campaignId of campaignIds) {
    const queries = qc
      .getQueryCache()
      .findAll({ queryKey: ["email-blast-inbox", campaignId] });
    ...
    for (const query of queries) {
      ...
      qc.setQueryData(
        ...
        (
          old:
            | { success: boolean; emails: InboundEmail[]; total: number }
            | undefined,
        ) => {
          if (!old) {
            return old;
...
function updateAllInboxCaches(
  ...
  const listQueries = qc
    .getQueryCache()
    .findAll({ queryKey: ["email-blast-all-inbox"] });
    ...
    qc.setQueryData(
      ...
      (
        ...
        return {
          ...old,
          emails:
            typeof limit === "number" ? nextEmails.slice(0, limit) : nextEmails,
          total: exists ? old.total : old.total + 1,
        };
      },
    );
...
function handleEventNotifications(
  data: WSEvent,
  add: ReturnType<typeof useNotifications>["add"],
) {
  switch (data.type) {
    ...
    case "got_number":
      add({
        type: "got_number",
        title: "Secretariat number found!",
    ...
    case "blast_completed": {
      ...
      add({
        ...
        title:
          failedCount === 0
            ? "Blast campaign done!"
            : `Blast done — ${failedCount} failures`,
        body:
          failedCount === 0
            ? "All messages sent successfully."
            : `${failedCount} recipients failed.`,
      });
      break;
    }
    case "blast_paused":
    ...
    case "blast_resumed":
      add({
        ...
        title: "Blast resumed",
        body:
          data.source === "auto_resume"
            ? "Campaign resumed automatically."
            : "Campaign resumed.",
      });
      break;
    case "quota_reached":
      add({
        ...
        body: `Only ${data.remaining} conversations remaining.`,
      });
      break;
    ...
    case "university_updated":
      add({
        ...
        body: `University #${data.uni_id} → ${data.status}`,
      });
      break;
  }
}
  ...
  switch (data.type) {
    ...
    case "conversation_changed":
      qc.invalidateQueries({
        queryKey: queryKeys.conversations.detail(data.conv_id),
      });
    ...
    case "got_number":
      ...
      qc.invalidateQueries({
        queryKey: queryKeys.universities.detail(data.uni_id),
      });
    ...
    case "quota_exhausted":
      qc.invalidateQueries({
        queryKey: ["email-blast-campaign", data.campaign_id],
      });
      break;
    case "marketing_client_updated":
      ...
      break;
    case "marketing_search_completed":
      void qc.invalidateQueries({
...
function onMessage(event: MessageEvent) {
  try {
    ...
    if (data.type === "log_line") {
      _logLineSubscribers.forEach((cb) =>
        cb({ ts: data.ts, level: data.level, text: data.text }),
      );
      return;
    }
    if (queryClientInstance) handleEventQuery(data, queryClientInstance);
    if (notificationsInstance)
      handleEventNotifications(data, notificationsInstance.add);
  } catch {
    // ignore parse errors
  }
}
...
function connectWs() {
  ...
  wsInstance.onclose = (e) => {
    ...
    if (e.code !== 1000 && wsSubscriberCount > 0)
