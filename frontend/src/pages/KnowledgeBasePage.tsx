...
import { KNOWLEDGE_TOUR_STEPS } from "../tours/knowledge.tour";
...
const TABS: { key: ChatbotType; label: string; configKey: string }[] = [
  {
    ...
    configKey: "AGENT_CUSTOM_INSTRUCTIONS",
  },
  {
    key: "audiensi",
    label: "Audiensi",
    configKey: "AUDIENSI_CUSTOM_INSTRUCTIONS",
  },
];
...
function CustomInstructionsEditor({ configKey }: { configKey: string }) {
  ...
  const isDirty =
    localValue !== null && localValue !== ((setting?.value as string) ?? "");

  const handleSave = () => {
    if (localValue === null) return;
    updateConfig.mutate(
      { [configKey]: localValue },
      {
...
function KnowledgeItemForm({
      ...
      updateMutation.mutate(
        {
          ...
          situation_tags: situationTags.trim(),
  ...
  return (
    <form
      ...
      className="rounded-lg border border-indigo-200 bg-indigo-50/50 p-4 dark:border-indigo-800 dark:bg-indigo-950/20"
    >
      <div className="space-y-3">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">
            Kata kunci yang memicu inject item ini. Jika pesan kontak mengandung
            salah satu keyword, item akan di-inject ke system prompt.
          </p>
        </div>
        <div>
          <input
...
function FileUploadZone({
  ...
  return (
    ...
    >
      ...
      />
      <Upload
        className={`mx-auto h-6 w-6 ${dragOver ? "text-indigo-500" : "text-gray-400"}`}
      />
...
function ChatbotKBSection({
  ...
  return (
    <div className="space-y-6">
      {/* Custom Instructions */}
      <div data-tour="kb-instructions">
        <CustomInstructionsEditor configKey={configKey} />
      </div>

      {/* Knowledge Items */}
      <div data-tour="kb-items">
...
export default function KnowledgeBasePage() {
  ...
  usePageTour("knowledge", KNOWLEDGE_TOUR_STEPS);
