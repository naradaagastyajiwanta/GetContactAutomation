...
def _test_proxy_connectivity(proxy_url: str, timeout: float = 3.0) -> bool:
    """Verify the SOCKS5 proxy can actually route traffic, not just accept TCP connections.

    A TCP connection to gost/SOCKS5 succeeds even when the upstream tunnel is
    broken (e.g. WARP data plane down — gost returns 503). This probes a real
    SOCKS5 handshake + HTTP GET so we only use the proxy when it truly works.
    """
    try:
        ...
        with socket.create_connection((ip, port), timeout=timeout) as sock:
            ...
            start = sock.recv(12)
            return start.startswith(b"HTTP/")
    except Exception:
        return False
...
def _resolve_ddg_proxy() -> str | None:
    ...
    Uses a real SOCKS5+HTTP probe so a listening-but-broken proxy (e.g. WARP
    data plane down) is correctly detected and skipped. Cache 60 s.
    """
    ...
    for port, proxy_url in candidates:
        ...
        if _test_proxy_connectivity(proxy_url, timeout=3.0):
            log.debug("[DDG] Proxy %s healthy, using it", proxy_url)
            _proxy_probe_result = proxy_url
            return _proxy_probe_result
        log.debug("[DDG] Proxy %s TCP-open but not routing, skipping", proxy_url)

    log.debug("[DDG] No working proxy, using direct connection")
